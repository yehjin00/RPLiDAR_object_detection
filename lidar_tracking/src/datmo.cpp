...
Datmo::Datmo(){
  ...
  n_private.param("world_frame", world_frame, string("laser"));
...
void Datmo::visualiseGroupedPoints(const vector<pointList>& point_clusters){
  ...
  for(unsigned int i=0; i<point_clusters.size(); ++i){
    ...
    if(gpoints.points.size()>1) marker_array.markers.push_back(gpoints);

        //////////////////////////////////////////////////////////////
        // Text marker
        //////////////////////////////////////////////////////////////
        visualization_msgs::Marker text_msg;
        text_msg.header.frame_id = world_frame;
        text_msg.header.stamp = ros::Time::now();
        text_msg.ns = "text";
        text_msg.id = gpoints.id;
        text_msg.type = visualization_msgs::Marker::TEXT_VIEW_FACING;
        text_msg.action = visualization_msgs::Marker::ADD;
        text_msg.pose.position.x = point_clusters[i][j].first + 0.5;
        text_msg.pose.position.y = point_clusters[i][j].second + 0.5;
